 ...
 *   WebRtcAgcState agc;
 *   webrtc_agc_init(&agc);
 *   webrtc_agc_process(&agc, float_buf_480, 480);  // in-place, 48kHz
 *
 ...
 */
static inline void webrtc_agc_apply_digital_gains_48k(
    const int32_t gains[11],
    int16_t* buf,
    int len)
{
    const int L = len / 10; /* 48 samples per sub-frame */
    const int L2 = 5;       /* log2(32) ~= shift for delta; adjusted for 48 */
    int k;
    size_t n;

    /* first sub-frame */
    {
        int32_t delta = (gains[1] - gains[0]) / L;
        int32_t gain32 = gains[0] * (1 << 4);
        int32_t delta_shifted = delta * (1 << 4);
        for (n = 0; n < (size_t)L; n++) {
            int64_t tmp64 = ((int64_t)buf[n]) * (gain32 >> 4);
            tmp64 >>= 16;
            if (tmp64 > 32767) {
                buf[n] = 32767;
            } else if (tmp64 < -32768) {
...
typedef struct {
    ...
    int      sampleRate;
    int16_t  tmpBuf16k[160];       /* downsample buffer */
    int16_t  tmpBuf[480];          /* float<->int16 buffer */
} WebRtcAgcState;
...
/*
 * Process 480 float samples in-place at 48kHz.
 * 1. float -> int16
 * 2. downsample 480@48k -> 160@16k
 * 3. compute gains on 160 samples
 * 4. apply gains to 480 samples
 * 5. int16 -> float
 */
    ...
    WebRtcAgcState* st, float* buf, int len)
{
    ...
    /* 2. downsample 480@48k -> 160@16k (average every 3 samples) */
    for (i = 0; i < 160; i++) {
        st->tmpBuf16k[i] = (int16_t)(
            ((int32_t)st->tmpBuf[i * 3] +
             (int32_t)st->tmpBuf[i * 3 + 1] +
             (int32_t)st->tmpBuf[i * 3 + 2]) / 3);
    }

    /* 3. compute gains */
    webrtc_agc_compute_digital_gains(
        &st->digital, st->tmpBuf16k, 160, gains);

    /* 4. apply gains to full 480 samples at 48kHz */
    webrtc_agc_apply_digital_gains_48k(gains, st->tmpBuf, len);
